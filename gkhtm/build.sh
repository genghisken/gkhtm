...
Darwin)  echo "Build for MacOS"
    ...
    c++ -c HTMCircleRegion.cpp -I./htm/include ${CLANG_FLAGS}
    c++ -c gkhtm_wrap.cxx -I./htm/include ${CLANG_FLAGS} -fno-strict-aliasing `python${PYTHON_VERSION}-config --includes`
    c++ -bundle -undefined dynamic_lookup -o _gkhtm.so HTMCircleRegion.o gkhtm_wrap.o -lhtm -L./htm -arch x86_64 -L`python${PYTHON_VERSION}-config --prefix`/lib
    ;;
linux)  echo  "Build for Linux"
    ...
    c++ -c HTMCircleRegion.cpp -I./htm/include ${CFLAGS}
    c++ -c gkhtm_wrap.cxx -I./htm/include ${CFLAGS} -fno-strict-aliasing `python${PYTHON_VERSION}-config --includes`
    c++ -shared  -o _gkhtm.so HTMCircleRegion.o gkhtm_wrap.o -lhtm -L./htm -L`python${PYTHON_VERSION}-config --prefix`/lib
    ;;
*) echo "Unsupported OS"
   ;;
esac
